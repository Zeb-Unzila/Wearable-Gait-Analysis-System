...
def generate_gait_session(
    filename="gait_session_log.csv",
    duration_sec=10,
    sample_rate_hz=50
):
    nodes = {
    ...
    with open(filename, mode="w", newline="") as file:
        ...
        for step in range(total_samples):
            current_time = (
                start_timestamp +
                (step * (1000 // sample_rate_hz))
            )

            for node_id, location in nodes.items():

                # Simulated biomechanical gait acceleration
    ...
    print(
        f"Session dataset successfully generated: "
        f"'{filename}'"
    )
